...
public interface ProductRepository extends JpaRepository<Product, UUID> {
    ...
    @Query(value = "SELECT p.* FROM product p WHERE p.is_deleted = false", nativeQuery = true)
    Page<Product> findAllNotDeleted(final Pageable pageable);

    @Query(value = "SELECT p.* FROM product p WHERE cast(p.price -> 'value' as float) BETWEEN :min AND :max AND p.is_deleted = false", nativeQuery = true)
    ...
    @Query(value = "SELECT p.* FROM product p WHERE p.name LIKE %:name% AND p.is_deleted = false", nativeQuery = true)
    Page<Product> findByNameAndNotDeleted(@Param("name") final String name, final Pageable pageable);

    @Query(value = "SELECT p.* FROM product p WHERE cast(p.price -> 'value' as float) < :price AND p.is_deleted = false", nativeQuery = true)
    ...
    @Query(value = "SELECT p.* FROM product p WHERE p.name LIKE %:name% AND cast(p.price -> 'value' as float) < :price AND p.is_deleted = false", nativeQuery = true)
    ...
    @Query(value = "SELECT p.* FROM product p WHERE p.name LIKE %:name% AND cast(p.price -> 'value' as float) BETWEEN :min AND :max AND p.is_deleted = false", nativeQuery = true)
