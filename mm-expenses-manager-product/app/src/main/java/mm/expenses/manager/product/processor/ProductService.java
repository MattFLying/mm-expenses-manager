...
public class ProductService {
    ...
    private final Map<BaseProductHandler.Type, BaseProductHandler<?>> handlers;
    ...
    public Product create(final CreateProductRequest request) {
        ...
        final var handler = getHandler(BaseProductHandler.Type.CREATE);
        final var result = handler.handle(requestedData);

        return (Product) result.getResponse();
    }
    ...
    public ProductResponse create(final CreateProductRequest request, final CurrencyCode defaultCurrency) {
        ...
        return result.getDecoratedResponse();
    }
    ...
    public Product update(final UUID productId, final UpdateProductRequest request) {
        ...
        final var handler = getHandler(BaseProductHandler.Type.UPDATE);
        final var result = handler.handle(requestedData);
    ...
    public void updateMissingCurrencies() {
        final var handler = getHandler(BaseProductHandler.Type.UPDATE_PRICE_CURRENCIES);
        handler.handle(null);
    }

    @Transactional
    public void hardDelete() {
        final var handler = getHandler(BaseProductHandler.Type.HARD_DELETE);
        handler.handle(null);
    }

    public Product delete(final UUID productId) {
        final var requestedData = requestOf(productId);
        final var handler = getHandler(BaseProductHandler.Type.DELETE);
        final var result = handler.handle(requestedData);
    ...
    public void delete(final Set<UUID> productIds) {
        ...
        final var handler = getHandler(BaseProductHandler.Type.DELETE_MANY);
    ...
    public Product findById(final UUID productId, final Boolean isDeleted) {
        ...
        final var handler = getHandler(BaseProductHandler.Type.FIND);
        final var result = handler.handle(requestedData);
    ...
    public Page<ProductResponse> search(final EntityFilter queryFilter) {
        ...
        final var handler = getHandler(BaseProductHandler.Type.SEARCH);
        final var result = handler.handle(requestedData);

        return (Page<ProductResponse>) result.getPagedResponse();
    }
    ...
    public ProductPage search(final EntityFilter queryFilter, final CurrencyCode defaultCurrency) {
        ...
        return result.getDecoratedPagedResponse();
    }
