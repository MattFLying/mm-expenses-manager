...
class ProductSchedulerTest extends ProductApplicationTest {
    ...
    void shouldDeleteMarkedAsDeleted() {
        ...
        // when
        when(productRepository.findAllByIsDeletedTrue(eq(PageFactory.getPageRequest(0, 50)))).thenReturn(new PageImpl<>(List.of(existed_1, existed_2)));
        when(productRepository.findAllByIsDeletedTrue(eq(PageFactory.getPageRequest(1, 50)))).thenReturn(new PageImpl<>(List.of()));

        productScheduler.cleanDeletedProducts();

        // then
        verify(productRepository, times(2)).findAllByIsDeletedTrue(any());
        verify(productRepository).deleteByIdIn(anyList());
    }
