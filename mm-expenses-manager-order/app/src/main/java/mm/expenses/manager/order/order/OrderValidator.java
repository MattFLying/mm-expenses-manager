...
class OrderValidator implements Validator<Order, CreateNewOrderRequest, UpdateOrderRequest> {
    ...
    @Override
    public Collection<ValidatorMessage> validateNew(final CreateNewOrderRequest object) {
        final var validators = new ArrayList<ValidatorMessage>();
        validators.addAll(validateName(object.getName()));
    ...
    @Override
    public Collection<ValidatorMessage> validateUpdate(final UpdateOrderRequest object) {
        final var validators = new ArrayList<ValidatorMessage>();
        validators.addAll(validateName(object.getName()));
    ...
    private Collection<ValidatorMessage> validateNewOrderedProducts(final List<CreateNewOrderedProductRequest> objects) {
        return objects.stream()
    ...
    private Collection<ValidatorMessage> validateUpdatedOrderedProducts(final List<UpdateOrderedProductRequest> objects) {
        return objects.stream()
                .map(orderedProduct -> validateProductQuantity(orderedProduct.getId(), orderedProduct.getQuantity()))
                .flatMap(Collection::stream)
                .collect(Collectors.toList());
