...
class ProductValidator implements Validator<Product, CreateNewProductRequest, UpdateProductRequest> {
    ...
    @Override
    public Collection<ValidatorMessage> validateNew(final CreateNewProductRequest object) {
        return validate(object.getName(), priceMapper.map(object.getPrice()));
    }

    @Override
    public Collection<ValidatorMessage> validateUpdate(final UpdateProductRequest object) {
        return validate(object.getName(), priceMapper.map(object.getPrice()));
    }
